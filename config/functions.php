...
function nominal($jumlahnominal,$case)
{
  // return $jumlahnominal;

  if (strlen($jumlahnominal) > 9) {
    $nominal1 = number_format(($jumlahnominal/1000000000),2);
    $nominal = number_format(($jumlahnominal/1000000000),2);
    $penyebut = "Milyar";
  } else {
    $nominal1 = number_format(($jumlahnominal/1000000000),2);
    $nominal = number_format(($jumlahnominal/1000000),2);
    $penyebut = "Juta";
  }

  switch ($case){
    case 1: return $nominal1; break;
    case 2: return $nominal; break;
    case 3: return $penyebut; break;
  }
}
...
function dinamic_title($page,$id){
  if(isset($id)){
    $sql = mysqli_query($server,"select nama_produk from produk where id_produk='$id'");
...
function sensor($teks){
    $w = mysqli_query($server,"SELECT * FROM katajelek");
