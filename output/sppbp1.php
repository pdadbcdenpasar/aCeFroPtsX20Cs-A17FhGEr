...
$SISAsql = "select distinct(d.vol),e.*,d.jenis from (select *, \n"
." sum(CASE \n"
." WHEN Golongan = 'A' THEN '".$GolA['tarif']."' \n"
." WHEN Golongan = 'B' THEN '".$GolB['tarif']."' \n"
." ELSE '".$GolC['tarif']."' END) AS tarif  from (select a.Kode, a.Nama_Perusahaan, a.Volume, a.Golongan, b.Jml_dppc, a.Jml_ck1a, b.Jml_dppc-a.Jml_ck1a as saldo , IF(a.Kode IS NOT NULL,1,0) AS drafted \n"
...
." LEFT JOIN ( select dpita.Kode AS Kode,dpita.Nama_Perusahaan AS Nama_Perusahaan,dpita.Volume AS Volume, dpita.Golongan AS Golongan,if(sum(ck1a.Jumlah),sum(ck1a.Jumlah),0) AS Jml_ck1a \n"
." from (dpita left join ck1a on((dpita.Kode = ck1a.Kode))) where YEAR(ck1a.Tanggal) = YEAR(CURDATE())-1 group by dpita.Kode) a ON b.Kode = a.Kode) c \n"
." where saldo !=0 and Nama_Perusahaan = '".$perus['nm_perusahaan1']."' group by Kode) e \n"
." LEFT JOIN ( \n"
." SELECT * FROM sppbp WHERE nm_perusahaan = '".$perus['nm_perusahaan1']."' and vol in ( \n"
...
." from (dpita left join ck1a on((dpita.Kode = ck1a.Kode))) where YEAR(ck1a.Tanggal) = YEAR(CURDATE())-1 group by dpita.Kode) a ON b.Kode = a.Kode) d \n"
." where saldo !=0 and Nama_Perusahaan = '".$perus['nm_perusahaan1']."' group by Kode order by tgl_p3c desc) order by tgl_p3c desc) d on e.Volume = d.vol group by Kode";
...
<table align="left" border="0" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "12";}else{echo "12";}?>px;">
  <tr>
  ...
  <tr>
	...
	<td width="2">:</td>
    <td width="350"><?php $nppbkc = $data['nppbkc']; echo substr($nppbkc,0,4).".".substr($nppbkc,4,1).".".substr($nppbkc,5,1).".".substr($nppbkc,6,4);?></td>
  </tr>
    <tr>
...
<table align="left" border="0" style="border-collapse:collapse; font-family:Arial; font-size:11px;">
  <tr>
...
<?php 
$start = date('Y-12-01', strtotime("-2 year", strtotime(date("Y-m-d"))));
$end = date('Y-11-30', strtotime("-1 year", strtotime(date("Y-m-d"))));
$nomor  = 0;
while ($row2 = mysqli_fetch_array($stmt2)) 
{ $totalSaldo = $row2['saldo'];
$P3Csql = "SELECT * FROM `p3c` WHERE nm_perusahaan = '".$perus['nm_perusahaan1']."' and vol = '".$row2['Volume']."' and tgl_p3c between '".$start."' and '".$end."' order by tgl_p3c desc ";
$P3Cstmt = mysqli_query($server,$P3Csql)  or die ("Query P3C salah : ".mysqli_error());	
$P3Ctotal = 0;
while ($P3Crow = mysqli_fetch_array($P3Cstmt)) 
{ 	mysqli_query($server,"insert into sppbp (p3c_id, no_p3c, tgl_p3c, nm_perusahaan, jenis, gol, vol, jml_lembar) VALUES ('".$P3Crow['p3c_id']."','".$P3Crow['no_p3c']."','".$P3Crow['tgl_p3c']."','".$P3Crow['nm_perusahaan']."','".$P3Crow['jenis']."','".$P3Crow['gol']."','".$P3Crow['vol']."','".$P3Crow['jml_lembar']."')");
	$P3Ctotal += $P3Crow['jml_lembar'];
	if ($P3Ctotal >= $totalSaldo){
	break;}
}}?>	<table align="left" border="0" width="350px" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "11";}else{echo "11";}?>px; padding-top: -10px;">
<?php 
$P3Csql2 = "SELECT distinct(no_p3c),tgl_p3c,jenis FROM sppbp WHERE nm_perusahaan = '".$perus['nm_perusahaan1']."' order by tgl_p3c desc ";
$P3Cstmt2 = mysqli_query($server,$P3Csql2)  or die ("Query P3C salah : ".mysqli_error());	
while ($P3Crow2 = mysqli_fetch_array($P3Cstmt2)) 
{ 	$nomor++;?>
	<tr>
		<td align="center"><?php echo $nomor.".";?></td>
		<td>SAC-<?php echo tambah_nol($P3Crow2['no_p3c'],6);?></td>
		<td> TANGGAL </td>
		<td><?php echo Tanggalindo($P3Crow2['tgl_p3c']);?></td>
		
	</tr>
<?php }?>
</table>
<table align="left" border="0" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "12";}else{echo "12";}?>px; padding-top: -10px;">
<tr>
<td>dapat disampaikan rincian sisa persediaan pita cukai Minuman Mengandung Etil Alkohol sebagai berikut:</td>
</tr>
</table>
  <table align="left" border="1" width="350px" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "8";}else{echo "11";}?>px;">
	<tr bgcolor=#F0F8FF>
	...
	<tr>
		...
		<td align="center" width="5%"><?php echo substr($SISArow['jenis'],0,10);?></td>
...
<table align="left" border="0" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "12";}else{echo "12";}?>px; padding-top: -10px;">
  <tr>
    <td align="justify">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Sesuai ketentuan dalam pasal 25 Peraturan Direktur Jenderal Bea dan Cukai Nomor PER-45/BC/2016 tanggal 30 Desember 2016 tentang Penyediaan dan Pemesanan Pita Cukai, atas sisa pita cukai Minuman Mengandung Etil Alkohol yang tidak direalisasikan dengan CK-1A tersebut dikenakan biaya pengganti penyedia pita cukai, sehingga ditetapkan adanya tagihan yang harus Saudara lunasi dengan rincian sebagai berikut :</td>
</tr>
</table>
	<table align="left" border="1" width="600px" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "12";}else{echo "12";}?>px; -20px;">
	<tr>
		<td align="center">&nbsp;&nbsp;&nbsp;&nbsp; Jumlah Pita Cukai &nbsp;&nbsp;(<?php echo $SISAtotal; ?> &nbsp; lembar) &nbsp; x &nbsp; 60&nbsp; x &nbsp; Rp. 300,00 &nbsp;&nbsp;&nbsp; = &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
		<td align="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Rp.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo str_replace(",",".",number_format($SISAtotal*60*300)); ?>,00&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </td>
	</tr>
	<tr>
	...
	<table align="left" border="0" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "11";}else{echo "11";}?>px; padding-top: -10px;">
	<tr>
...
<table align="right" border="0" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "12";}else{echo "12";}?>px;">
	<tr>
...
<table align="left" border="1" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "11";}else{echo "11";}?>px;">
<tr>
...
<table align="left" border="0" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "11";}else{echo "12";}?>px;">
  <tr>
...
<p></p>
<table align="left" border="0" style="border-collapse:collapse; font-family:Arial; font-size:<?php if ($data['nppbkc']=="0802121012"){echo "11";}else{echo "11";}?>px;">
<tr>
